...
public Action:timer_CheckForTanks(Handle:timer)
{
	...
	while ((ent_tank = FindEntityByClassname(ent_tank, "tank_boss")) != -1)
	{
		if (ent_tank > 0)
		{
			...
			new currHealth = GetEntProp(ent_tank, Prop_Data, "m_iHealth");
//			PrintToChatAll("MvMBalancer: healthPenalty %f\tnewMaxHealth %d\tCurrHealth %d", healthPenalty, newMaxHealth, currHealth);
		
			//Hacky method of not lowering the health of the same tank twice.
			if(currHealth > newMaxHealth)
			{	
